  ...
  document.getElementById('tl-next').addEventListener('click', function () {
    ...
    if (isFinal) {
      ...
      fetch('/api/public/checkout', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ plan: plan, email: state.data.email || '', order_id: orderId, lang: 'en' })
      }).then(function (r) { return r.json(); }).then(function (j) {
        if (j && j.url) { window.location.href = j.url; }
        else { btn.disabled = false; btn.textContent = origText; alert((j && j.error) || 'Payment error'); }
      }).catch(function () {
        btn.disabled = false; btn.textContent = origText; alert('Payment error');
      });
      return;
    }
    state.step++;
...
(function(){
  function init(){
    document.addEventListener('click', function(e){
      ...
      try {
        localStorage.setItem('tl-pending-payment', JSON.stringify({
          order_id: orderId, amount: plan, currency: 'USD', email: '', plan: plan, lang: 'en'
        }));
      } catch(_){}
      fetch('/api/public/checkout', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ plan: plan, order_id: orderId, lang: 'en' })
      }).then(function(r){ return r.json(); }).then(function(j){
        if (j && j.url) { window.location.href = j.url; }
        else { a.textContent = orig; a.dataset.tlLoading = ''; alert((j && j.error) || 'Payment error'); }
      }).catch(function(){
        a.textContent = orig; a.dataset.tlLoading = ''; alert('Payment error');
      });
    });
  }
